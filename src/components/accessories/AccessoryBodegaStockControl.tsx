...
interface AccessoryBodegaStockControlProps {
  accessoryId: string
  initialStock: { bodega: string; stock: number }[]
}

export default function AccessoryBodegaStockControl({ accessoryId, initialStock }: AccessoryBodegaStockControlProps) {
  ...
  const adjust = async (bodega: string, newStock: number) => {
    ...
    setSaving(bodega)
    await adjustAccessoryBodegaStock(accessoryId, bodega, newStock)
    setSaving(null)
  }
