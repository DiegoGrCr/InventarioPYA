...
import { adjustMeshBodegaStock } from '@/actions/meshes'
import { useIsAdmin } from '@/contexts/AdminContext'

interface MeshBodegaStockControlProps {
  meshId: string
  initialStock: { bodega: string; stock: number }[]
}

export default function MeshBodegaStockControl({ meshId, initialStock }: MeshBodegaStockControlProps) {
  ...
  const adjust = async (bodega: string, newStock: number) => {
    ...
    setSaving(bodega)
    await adjustMeshBodegaStock(meshId, bodega, newStock)
    setSaving(null)
  }
