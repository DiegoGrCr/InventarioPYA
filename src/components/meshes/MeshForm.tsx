...
export default function MeshForm({ brands, sizes, mesh, bodegaStock = [] }: MeshFormProps) {
  const router = useRouter()
  const formRef = useRef<HTMLFormElement>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  const [preview, setPreview] = useState<string | null>(mesh?.image_url || null)
  const [imageUrl, setImageUrl] = useState<string | null>(mesh?.image_url || null)
  const [uploading, setUploading] = useState(false)
  const [uploadError, setUploadError] = useState('')
  const [pricePerSqm, setPricePerSqm] = useState(mesh?.price_per_sqm?.toString() || '')
  const [sqmPerBox, setSqmPerBox] = useState(mesh?.sqm_per_box?.toString() || '')
  const [piecesPerBox, setPiecesPerBox] = useState(mesh?.pieces_per_box?.toString() || '')
  const [saleUnit, setSaleUnit] = useState(mesh?.sale_unit || 'caja')
  ...
  const handleNameChange = (value: string) => {
    ...
    nameCheckTimeout.current = setTimeout(async () => {
      const matches = await findMeshesByName(value, mesh?.id)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = mesh
      ? await updateMesh(mesh.id, formData)
      : await createMesh(formData)
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="form-card">
      ...
      <div className="form-group">
        <label className="form-label">Foto de la malla</label>
        {uploadError && (
      ...
      <div className="form-group">
        <label className="form-label">Nombre de la malla *</label>
        <input
          type="text"
          name="name"
          ...
          placeholder="Ej: Malla de Fibra de Vidrio"
        ...
        {nameMatches.length > 0 && (
          <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', padding: '8px 12px', background: 'var(--warning-light)', color: 'var(--warning)', borderRadius: 'var(--radius-sm)', marginTop: '8px', fontSize: '13px' }}>
            ...
            <span>
              Ya existe {nameMatches.length === 1 ? 'una malla' : `${nameMatches.length} mallas`} con este nombre: {' '}
              {nameMatches.map((m, i) => (
                <span key={m.id}>
                  {i > 0 && ', '}
      ...
      <div className="form-row">
        <div className="form-group">
          ...
          <select name="brand_id" className="form-select" defaultValue={mesh?.brand_id || ''}>
        ...
        <div className="form-group">
          ...
          <select name="size_id" className="form-select" defaultValue={mesh?.size_id || ''}>
      ...
      <div className="form-row-3">
        <div className="form-group">
          ...
          <input type="text" name="sku" className="form-input" defaultValue={mesh?.sku || ''} placeholder="Código único" />
        </div>
        <div className="form-group">
        ...
        <div className="form-group">
          ...
          <input type="text" name="color" className="form-input" defaultValue={mesh?.color || ''} placeholder="Gris, Beige..." />
        </div>
      </div>
      ...
      <div className="form-row">
        ...
        <div className="form-group">
          ...
          <input type="text" name="finish" className="form-input" defaultValue={mesh?.finish || ''} placeholder="Mate, Brillante..." />
        </div>
      </div>
      ...
      <div className="form-group">
        ...
        <textarea name="description" className="form-textarea" defaultValue={mesh?.description || ''} placeholder="Características de la malla..." />
      </div>

      <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
        <button type="submit" className="btn btn-primary" disabled={loading || uploading}>
          {loading ? <><Loader2 size={15} className="spin" /> Guardando...</> : uploading ? <><Loader2 size={15} className="spin" /> Subiendo imagen...</> : mesh ? <><Save size={15} /> Actualizar</> : <><CheckCircle size={15} /> Crear Malla</>}
        </button>
