...
import { adjustProductBodegaStock } from '@/actions/products'
import { useIsAdmin } from '@/contexts/AdminContext'

interface ProductBodegaStockControlProps {
  productId: string
  initialStock: { bodega: string; stock: number }[]
}

export default function ProductBodegaStockControl({ productId, initialStock }: ProductBodegaStockControlProps) {
  ...
  const adjust = async (bodega: string, newStock: number) => {
    ...
    setSaving(bodega)
    await adjustProductBodegaStock(productId, bodega, newStock)
    setSaving(null)
  }
